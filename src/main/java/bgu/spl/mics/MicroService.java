...
public abstract class MicroService implements Runnable {
    //------Private-Fields--------
    private Diary diary;
    public MessageBusImpl MB;
    private String name;
    boolean terminated;
    private Map<Class<? extends Message>,Callback<?>> CBmessages;
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
    	MB.subscribeEvent(type, this); //Sending to MB_instance
        CBmessages.put(type,callback);
    }
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
    	MB.subscribeBroadcast(type,this); //Sending to MB_instance
    	CBmessages.put(type,callback);
    }
    ...
    protected final <T> Future<T> sendEvent(Event<T> e) {
        ...
        return this.MB.sendEvent(e); //Sending to MB_instance
    }
    ...
    protected final void sendBroadcast(Broadcast b) {
    	MB.sendBroadcast(b); //Sending to MB_instance
    }
    ...
    protected final <T> void complete(Event<T> e, T result) {
        MB.complete(e,result); //Sending to MB_instance
    }
    ...
    protected final void terminate() {
        this.terminated = true; //Changing terminated to true
        MB.unregister(this); //Unregistering MS
    }
    ...
    public final void run() {
        MB.register(this);
        this.initialize();
        while(!terminated){ //Running as long as this microservice hasnt finished
            try{
                Message message = MB.awaitMessage(this); //Waiting for message
                Callback callback = CBmessages.get(message.getClass()); //Retrieving Callback
                if (callback != null)
                    callback.call(message); //Executing callback
            }
